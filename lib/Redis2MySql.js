...
function Redis2MySql(options) {
  ...
  this.mysqlReadConn.connect(function (err) {
    ...
    console.log('connected as id ' + self.mysqlReadConn.threadId + '\n');
  });
  ...
  this.mysqlWriteConn.connect(function (err) {
    ...
    console.log('connected as id ' + self.mysqlWriteConn.threadId + '\n');
  });
...
Redis2MySql.prototype.quit = function () {
  ...
  this.mysqlReadConn.end(function (err) {
    console.log('Ending id ' + self.mysqlReadConn.threadId + '\n');
    if (err) {
      self.emit('error', {error: 'mysql', message: err.message});
    }
  });
  this.mysqlWriteConn.end(function (err) {
    console.log('Ending id ' + self.mysqlWriteConn.threadId + '\n');
    if (err) {
      self.emit('error', {error: 'mysql', message: err.message});
    }
  });
...
Redis2MySql.prototype.createUseSchema = function () {
  ...
  var self = this,
    ...
    sqlCreateSchema =
      'CREATE DATABASE IF NOT EXISTS ' + this.mysqlWriteConn.escapeId(this.options.mysql.database) +
      ' CHARACTER SET = ' + this.mysqlWriteConn.escape(this.options.mysql.charset),

    sqlUseSchema =
      'USE ' + this.mysqlWriteConn.escapeId(this.options.mysql.database);
