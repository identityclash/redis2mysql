      ...
      if (err) {
        ...
        self.mysqlConn.query(
          ...
          function (err, result) {
            if (err) {
              self.emit('error', {error: 'mysql', message: err.message});
            } else if (is.existy(result) && result[0]) {
          ...
          _createInsertUpdate(self.mysqlConn, sqlCreateListTable, sql,
            sqlParams, function (err, result) {
              ...
              if (err) {
                ...
                console.log('MySQL LPUSH result ' + result.message);
              }
            }
          );
        }
  ...
  if (!key) {
    ...
    this.redisConn.scard(redisKey, function (err, result) {
      if (err) {
        ...
        async.waterfall([
          function (firstCb) {
            var existsTableSql = 'SELECT EXISTS (SELECT 1 ' +
              'FROM information_schema.tables ' +
              'WHERE table_name = ?) AS table_exists';

            self.mysqlConn.query(existsTableSql, ordSetTableName,
              function (err, result) {
                if (err) {
                  firstCb(err);
                } else if (is.existy(result[0].table_exists)) {
                  firstCb(null, result[0].table_exists);
                }
              });
          },
          function (arg1, secondCb) {
            if (arg1 === 0) {
              secondCb(null, 0);
            } else {
              ...
              self.mysqlConn.query(sql, ordSetTableName, function (err, result) {
                if (err) {
                  secondCb(err);
                } else if (is.existy(result[0].cnt)) {
                  secondCb(null, result[0].cnt);
                }
              });
            }
          }
...
Redis2MySql.prototype.hset = function (hashKey, param1, param2, cb) {
  ...
  if (!(param1 && hashKey)) {
    ...
    self.redisConn.hget(redisHashKey, field, function (err, result) {
      if (err) {
        cb(err);
      } else {
            ...
            _createInsertUpdate(self.mysqlConn, sqlCreateHashTable, sql,
              sqlParams, function (err) {
                ...
                if (err) {
                  ...
                  // for rollback purposes
                  self.redisConn.hset(redisHashKey, field, originalValue, function (err) {
                    if (err) {
                      cb(err);
                    }
                  });
            ...
            _createInsertUpdate(self.mysqlConn, sqlCreateHashTable, sql,
              sqlParams, function (err) {
                ...
                if (err) {
                  ...
                  // for rollback purposes
                  self.redisConn.hmset(redisHashKey, originalValues, function (err) {
                    if (err) {
                      cb(err);
                    }
                  });
...
Redis2MySql.prototype.hmget = function (hashKey, fields, cb) {
  ...
  if (!fields) {
    ...
    var self = this, i, hashTableName, sql, sqlParams = [], redisHashKey =
      _prefixAppender([self.options.custom.datatypePrefix.hash, hashKey], ':');
    ...
    this.redisConn.hmget(redisHashKey, fields, function (err, result) {
      if (err) {
        ...
        sql = new SqlBuilder().select(1).from(1).where(fields.length, 'OR').toString();
...
Redis2MySql.prototype.hdel = function (hashKey, fields, cb) {
  ...
  if (!fields) {
    ...
    this.redisConn.hget(redisHashKey, fields, function (err, result) {
      if (err) {
        cb(err);
      } else {
        ...
        this.redisConn.hdel(redisHashKey, fields, function (err, result) {
          if (err) {
            cb(err);
          } else if (is.existy(result)) {
            cb(null, result);
