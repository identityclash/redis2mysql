  ...
  if (!(param1 && type)) {
    ...
    self.redisConn.getset(redisKey, value, function (err, result) {
      if (err) {
        ...
        sqlParams = [stringTableName, key, value, COLUMNS.KEY,
          key, COLUMNS.VALUE, value];

        _createInsertUpdate(self.mysqlConn, sqlCreateStringTable,
          sql,
          sqlParams,
          function (err, result) {
            if (err) {
...
function _createInsertUpdate(connection, sqlCreate, sqlInsertUpdate, sqlParams,
  ...
  var sql = sqlCreate + '; ' + sqlInsertUpdate + ';';
  ...
  connection.query(
    ...
    function (err, result) {
      ...
      return callback(null, result[1]); // result[0] is the create table results
    });
