...
function Redis2MySql(options) {
  ...
  this.mysqlConn.connect(function (err) {
    ...
    console.log('connected as id ' + self.mysqlConn.threadId + '\n');
  });
...
Redis2MySql.prototype.quit = function () {
  ...
  this.mysqlConn.end(function (err) {
    console.log('Ending id ' + self.mysqlConn.threadId + '\n');
    if (err) {
      self.emit('error', {error: 'mysql', message: err.message});
    }
  });
...
Redis2MySql.prototype.incr = function (type, key, cb) {
  ...
  if (!key) {
    ...
    var self = this, redisKey, tableName, sqlCreateTable, sql,
      sqlParams,
      tableColumns = [self.mysqlConn.escapeId(COLUMNS.KEY), COLUMNS.VALUE];
    ...
    sqlCreateTable =
      ...
      '(' +
      self.mysqlConn.escapeId(COLUMNS.KEY) + ' VARCHAR(255) PRIMARY KEY, ' +
    ...
    async.series(
      [
        function (createTblCb) {
          console.log('Created table, SQL Create: ' + sqlCreateTable);
...
Redis2MySql.prototype.set = function (type, param1, param2, cb) {
  ...
  if (!(param1 && type)) {
    ...
    async.series(
      [
        function (createTblCb) {
          console.log('Created table, SQL Create: ' + sqlCreateStringTable);
          self.mysqlConn.query(
            sqlCreateStringTable,
            function (err) {
              if (err) {
        ...
        function (redisCb) {
          console.log('redis input: ' + redisKey + ' value: ' + value);
        ...
        var tableColumns = [self.mysqlConn.escapeId(COLUMNS.KEY), COLUMNS.VALUE],
          sql, sqlParams;
        ...
        console.log('SQL for insert / update: ' + sql);
        ...
        self.mysqlConn.query(
          ...
          function (err, result) {
            if (err) {
              ...
              console.log('Redis SET MySQL result: ' + result.affectedRows + ' value: ' + value);
            }
          });
      });
  }
  ...
  if (!(key)) {
    ...
    this.redisConn.exists(key, function (err, result) {
      if (err) {
        ...
        var sql, sqlParams = [], lazySeq, dataArray, tableName, tableKey;

        if (lazy(key).contains(':')) {
          lazySeq = lazy(key).split(':');
        ...
        if (is.existy(tableKey)) {
          sql = new SqlBuilder().select(['COUNT(1) AS cnt']).from(1).where(1)
            .toString();
          sqlParams = [
            tableName,
            COLUMNS.KEY,
          ...
          sql = new SqlBuilder().select(['COUNT(1) AS cnt']).from(1).toString();
          sqlParams = [
            tableName
          ];
        }
                      ...
                      self.mysqlConn.query(sqlDeleteTable,
                        ...
                        function (err, result) {
                          if (is.not.existy(err)) {
                            console.log('Redis DEL MySQL row ' + tableKey +
...
Redis2MySql.prototype.lpush = function (key, values, cb) {
  ...
  if (!(key && values)) {
    ...
    async.series(
      [
        function (createTblCb) {
          console.log('Created table, SQL Create: ' + sqlCreateListTable);
          self.mysqlConn.query(
            sqlCreateListTable,
            function (err) {
              if (err) {
        ...
        function (redisCb) {
          console.log('redis input: ' + redisKey + ' values : ' + values);
          ...
          self.mysqlConn.query(
            ...
            function (err, result) {
              if (err) {
                ...
                console.log('Redis LPUSH MySQL result ' + result.affectedRows + ' value: ' + values);
              }
            });
        }
      });
...
Redis2MySql.prototype.sadd = function (key, members, cb) {
  ...
  if (!(key && members)) {
    ...
    async.series(
      [
        function (createTblCb) {
          console.log('Created table, SQL Create: ' + sqlCreateOrdSetTable);
          self.mysqlConn.query(
            sqlCreateOrdSetTable,
            function (err) {
              if (err) {
        ...
        function (redisCb) {
          console.log('redis input: ' + redisKey + ' members: ' + members);
        ...
        self.mysqlConn.query(
          ...
          function (err, result) {
            ...
            if (err) {
              ...
              console.log('Redis SREM MySQL result: ' + result.affectedRows);
            }
          }
        );
      }
...
Redis2MySql.prototype.zadd = function (key, scoreMembers, cb) {
  ...
  if (!(key && scoreMembers)) {
    ...
    async.series(
      [
        function (createTblCb) {
          console.log('Created table, SQL Create: ' + sqlCreateSortedSetTable);
          self.mysqlConn.query(
            sqlCreateSortedSetTable,
            function (err) {
              if (err) {
        ...
        function (redisCb) {
          console.log('redis input: ' + redisKey + ' scoreMembers: ' + scoreMembers);
      ...
      function (err, result) {
        ...
        console.log('SQL for insert / update: ' + sql);
        console.log('SQL for insert / update parameters: ' + sqlParams.toString());
        console.log('time elapsed: ' + (new Date().getTime() - startingTime));
        self.mysqlConn.query(
          sql,
          sqlParams,
          function (err, result) {
  ...
  if (!(key && incrDecrValue && member)) {
    ...
    async.series(
      [
        ...
        function (redisCb) {
          console.log('redis input: ' + redisKey + ' incrDecrValue: ' + incrDecrValue + ' member: ' + member);
        ...
        self.mysqlConn.query(
          ...
          function (err, result) {
            if (err) {
              ...
              console.log('Redis ZINCRBY MySQL result: ' + result.affectedRows);
            }
          });
      });
  }
        ...
        self.mysqlConn.query(
          ...
          function (err, result) {
            if (err) {
              ...
              cb(null, is.existy(result[0]) ? result[0].redis_index : null);
            }
          }
        );
      }
      ...
      if (err) {
        ...
        var originalValue = result,
          ...
          sqlCreateHashTable =
            ...
            '(' +
            self.mysqlConn.escapeId(COLUMNS.FIELD) + ' VARCHAR(255) PRIMARY KEY, ' +
            COLUMNS.VALUE + ' VARCHAR(255), ' +
            COLUMNS.CREATION_DT + ' TIMESTAMP(3) DEFAULT NOW(3), ' +
            COLUMNS.LAST_UPDT_DT + ' TIMESTAMP(3) DEFAULT NOW(3) ON UPDATE NOW(3)' +
            ') ';
        ...
        async.series(
          [
            function (createTblCb) {
              console.log('Created table, SQL Create: ' + sqlCreateHashTable);
              self.mysqlConn.query(
                sqlCreateHashTable,
                function (err) {
                  if (err) {
          ...
          function (err, result) {
            ...
            var tableColumns = [self.mysqlConn.escapeId(COLUMNS.FIELD), COLUMNS.VALUE],
              sql, sqlParams;
          ...
          function (err, result) {
            ...
            var tableColumns = [self.mysqlConn.escapeId(COLUMNS.FIELD), COLUMNS.VALUE],
              ...
              sql = new SqlBuilder().insertIgnore('??', tableColumns)
                ...
                .onDuplicate({value: 'VALUES(' + COLUMNS.VALUE + ')'});
...
Redis2MySql.prototype.hmget = function (hashKey, fields, cb) {
  ...
  if (!fields) {
    ...
    this.redisConn.hmget(redisHashKey, fields, function (err, result) {
      ...
      var i, isNullArray = true, hashTableName, sqlSelect, sql, sqlParams = [],
        fieldParams = '';

      for (i = 0; i < result.length; i++) {
        if (is.existy(result[i])) {
          isNullArray = false;
          break;
        }
      }
      ...
      if (err) {
        cb(err);
      } else if (is.existy(result) && !isNullArray) {
        cb(null, result);
      } else {
        ...
        sql = ('SELECT ' + COLUMNS.VALUE + ' FROM ( ' + sqlSelect +
                ...
                self.mysqlConn.query(
                  ...
                  function (err) {
                    if (is.not.existy(err)) {
                      ...
                      console.log('Table does not exist: ' + TABLE_EXPIRY);
                      thirdCb();
                    } else {
                      thirdCb(err);
                    }
...
Redis2MySql.prototype.expire = function (key, seconds, cb) {
  ...
  if (!(key && seconds)) {
    ...
    async.series(
      [
        function (createTblCb) {
          console.log('Created table, SQL Create: ' + sqlCreateExpiryTable);
          self.mysqlConn.query(
            sqlCreateExpiryTable,
            function (err) {
              if (err) {
...
function _createInsertUpdate(connection, sqlCreate, sqlInsertUpdate, sqlParams,
                             callback) {

  connection.query(
    sqlCreate + '; ' + sqlInsertUpdate + '; ',
    sqlParams,
    function (err, result) {
      if (err) {
        return callback(err);
