  ...
  if (!key) {
    ...
    self.redisConn.incr(redisKey, function (err, result) {
      if (err) {
        ...
        var COLUMNS = [self.mysqlConn.escapeId(COLUMNS.KEY), COLUMNS.VALUE],
  ...
  if (!(param1 && type)) {
    ...
    self.redisConn.getset(redisKey, value, function (err, result) {
      if (err) {
        ...
        cb(null, is.existy(result) ? 'OK' : null);
    ...
    self.redisConn.multi().time().lpush(redisKey, values).exec(function (err, result) {
      ...
      if (err) {
        ...
        if (result[1][1] > 0) {
          ...
          var COLUMNS = [COLUMNS.SEQ, COLUMNS.VALUE], i, sqlParams = [],
  ...
  if (!(key && members)) {
    ...
    self.redisConn.sadd(redisKey, members, function (err, result) {
      ...
      if (err) {
        ...
        var i, COLUMNS = [COLUMNS.MEMBER], sql, sqlParams = [], ordSetTableName,
  ...
  if (!(key && scoreMembers)) {
    ...
    self.redisConn.zadd(redisKey, scoreMembers, function (err, result) {
      ...
      if (err) {
        ...
        var COLUMNS = [COLUMNS.SCORE, COLUMNS.MEMBER], sortedSetTableName,
...
Redis2MySql.prototype.zincrby = function (key, incrDecrValue, member, cb) {
  ...
  if (!(key && incrDecrValue && member)) {
    ...
    var self = this, COLUMNS = [COLUMNS.SCORE, COLUMNS.MEMBER], redisKey,
      ...
      if (err) {
        ...
        self.redisConn.hset(redisHashKey, field, value, function (err, result) {
          if (err) {
            ...
            var COLUMNS = [self.mysqlConn.escapeId(COLUMNS.FIELD), COLUMNS.VALUE],
