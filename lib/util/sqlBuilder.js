...
SqlBuilder.prototype.insert = function (tableName, columnsOrColumnCount) {
  ...
  if (Array.isArray(columnsOrColumnCount) && columnsOrColumnCount.length > 0) {

    this.sql += '(';

    for (name in columnsOrColumnCount) {
      if (columnsOrColumnCount.hasOwnProperty(name)) {
        this.sql += columnsOrColumnCount[name] + ' , ';
      }
    }
  ...
  } else if (is.number(columnsOrColumnCount) && columnsOrColumnCount > 0) {

    this.sql += '( ';

    for (i = 0; i < columnsOrColumnCount; i++) {
      this.sql += '??, ';
    }
    this.sql = this.sql.substring(0, this.sql.length - ', '.length);
...
SqlBuilder.prototype.replace = function (tableName, columns) {

  this.sql = this.insert(tableName, columns).toString().replace('INSERT INTO ', 'REPLACE INTO ');
